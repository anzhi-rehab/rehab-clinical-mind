...
本 Skill 不自建记忆库，不在后台训练模型。`rehab-clinical-mind / therapist-local-alias` 只是逻辑命名空间，不意味着平台已实现隔离。共享账号、多人会话或归属冲突时确认当前画像属于谁，只需非实名别名，不合并其他人的习惯。
...
首次简短说明默认会话内使用，询问/沿用长期画像选择；不重复请求已有授权。病例长期保存须明确选择该去标识摘要或已选择此类自动摘要；无授权仅输出草稿。拒绝记忆不影响任何模式。
...
读取references/profile-data.md，使用templates/PERSONAL_PROFILE.json和TRAINING_LOG.json，旧Markdown画像继续兼容。长期支持数按独立事件去重，真实来源练习仍为educational，不能算actual-clinical。A/B/C/D是特定主张的证据与风险分类，不替代偏好置信度。默认不填写当前用户的个人资料进公开模板。
